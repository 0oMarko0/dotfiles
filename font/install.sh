...
# Copied from: https://github.com/ryanoasis/nerd-fonts/blob/master/install.sh#L65
...
# Font are stored as zip file in the folder where install is located
nerdfonts_root_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
nerdfonts_dirs=("$nerdfonts_root_dir")

# -n true if the name of the string is non-zero
# "$* " is a single string that contains of all of the positional parameters,
if [ -n "$*" ]; then
  nerdfonts_dirs=()
  # "${@}" is all the argument pass to the script
  for font in "${@}"; do
    if [ -n "$font" ]; then
      # ensure the zip file exist in the directory
      if [[ ! -a "$font.zip" ]]; then
        echo -e "Font $font.zip doesn't exist"
        exit 255
      fi
...
# unzip selected file
for zipFile in "${nerdfonts_dirs[@]}"; do
  unzip -o -qq "$zipFile.zip" -d "$zipFile"
done
...
# which nerd font variant to install
if [ "$variant" = "M" ]; then
...
usr_font_dir="$HOME/.local/share/fonts/NerdFonts"
